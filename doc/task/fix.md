...
| ID | Vấn đề | File | Nguồn | Độ tin cậy |
|---|---|---|---|---|
| FIX-008 | `mAdapter` field không bao giờ gắn vào `RecyclerView` thật; mọi lần filter tạo adapter mới thay thế, mất scroll position, việc `notifyDataSetChanged()`/`filterList()` gọi trên field cũ là vô nghĩa | `act/MainAct.kt:63,95-96,334-336`; `act/table/DictionaryAct.kt:40,207-208,254-260`; `act/IsotopesActExperimental.kt:41,83-84,268-274`; `act/table/EquationsAct.kt:35,152-153,202-204`; `act/table/ElectrodeAct.kt:31,139-140,188-190`; `act/table/IonAct.kt:36,193-194,223-225` | claude + tự audit | 🔶 Cross-confirmed |
| FIX-009 | `nextPrev()` nuốt lỗi im lặng khi ở biên (Hydrogen/Oganesson): `elements[currentNumb.toInt()]` / `elements[currentNumb.toInt() - 2]` ra ngoài mảng, nhưng bị `catch (e: Exception)` bắt và chỉ `printStackTrace()` — **không crash** (đã verify, khác với báo cáo ban đầu của claude là "crash thật") — nút Next/Previous chỉ im lặng không làm gì, không có feedback cho user | `act/ElementInfoAct.kt:220-266` | codex (đúng), claude (sai — báo nhầm là crash) | ✅ Verified (đã đọc lại code) |
| FIX-010 | `catch (_: IOException)` không bắt được `JSONException` khi parse JSON lỗi cú pháp → crash không kiểm soát | `act/IsotopesActExperimental.kt:375` (và pattern tương tự ở `InfoExt.kt:366`) | codex, agy | 🔶 Cross-confirmed |
| FIX-011 | `mailto:` URI ghép trực tiếp title/content người dùng nhập không qua `Uri.encode()` — dấu cách, tiếng Việt có dấu, `#`/`&`/`?` phá hỏng subject/body; thiếu try/catch `ActivityNotFoundException` | `act/setting/SubmitAct.kt:203-211` | agy, claude, tự audit | 🔶 Cross-confirmed (3 nguồn) |
| FIX-012 | `RoyApp.pendingAdInitCallbacks` giữ lambda capture Activity (vd. `SplashAct`) không có API huỷ khi Activity destroy; state init ads (`adsInitializeStarted`) không đồng bộ, callback SDK có thể chạy ngoài main thread → race condition | `RoyApp.kt:16,36,88-107`; `act/SplashAct.kt:54-57,199-224` | codex, claude | 🔶 Cross-confirmed |
| FIX-013 | Banner VIP chỉ được check ẩn/hiện trong `onCreate()`, không re-check ở `onResume()` — redeem VIP trong `VipManagementAct` rồi back lại `ElementInfoAct`/`SettingsAct`/`FavoritePageAct` vẫn thấy banner cũ | `act/ElementInfoAct.kt:135`, `act/SettingsAct.kt:250`, `act/setting/FavoritePageAct.kt:303` | claude | ⚪ Cần verify |
| FIX-014 | Random Element và kết quả Search bypass interstitial gate, không nhất quán với click từ grid chính | `act/MainAct.kt:280-290,339-345` vs `:517-522` | claude | ⚪ Cần verify |
| FIX-015 | So khớp số oxi hoá bằng `contains("0")`/`contains("1")` trên toàn chuỗi → false positive (vd `"+10"` khớp nhầm `"+1"`) | `ext/InfoExt.kt:258-319` | agy, tự audit | 🔶 Cross-confirmed |
| FIX-016 | `loadImage()` tạo mới `OkHttpClient` + `Picasso` instance ở **mỗi lần gọi** (mỗi lần mở `ElementInfoAct` hoặc bấm Next/Previous), không bao giờ shutdown → rò connection pool/thread pool | `ext/InfoExt.kt:388-401` | tự audit | ⚪ Cần verify |
| FIX-017 | `onBindViewHolder` đọc + parse lại JSON asset ở **mỗi lần bind/recycle** thay vì cache; khi parse lỗi, catch chỉ `printStackTrace()` không reset text → item bị recycle hiển thị nhầm giá trị của item trước | `adt/IonAdapter.kt:53-77` | tự audit, claude (phần đọc asset đồng bộ) | 🔶 Cross-confirmed |
| FIX-018 | Parser công thức hoá học chấp nhận cặp ngoặc sai loại (`Ca(OH]2`, `{H2O)`) vì stack chỉ lưu map nguyên tố, không lưu loại ngoặc mở | `util/ChemicalFormulaParser.kt:94-125` | codex | ⚪ Cần verify |
| FIX-019 | Splash 30s "emergency fallback" gọi lại `goToMain()` nhưng hàm này thoát sớm nếu `isFullscreenAdShowing == true`, và không lên lịch timeout mới nếu listener đã tồn tại → vẫn có nguy cơ kẹt màn hình splash vô hạn trong trường hợp hiếm | `act/SplashAct.kt:157-184` | codex | ⚪ Cần verify |
...
## Phát hiện mới khi chạy instrumented test thật trên device (2026-09-01)

Chạy `connectedDevDebugAndroidTest` trên 2 thiết bị thật (TECNO KJ7, Pixel 7 Pro) lộ ra 2 vấn đề **có sẵn từ trước**, không liên quan tới 7 fix P0 — đã verify bằng cách chạy lại y hệt trên code gốc (trước khi có bất kỳ fix nào trong phiên này) cho kết quả giống hệt:

- **FIX-038 — `btnRevokeVip` không click được trên thiết bị thật (P2).** `VipManagementActTest`/`VipManagementWidgetTest`: 5 test liên quan revoke-flow fail với `view.getGlobalVisibleRect() covers at least 90 percent` — button tồn tại, enabled, visible nhưng Espresso coi là không đủ visible để click (`x=0.0, y=0.0` trong log, nghi ngờ do layout/inset chưa ổn định đúng lúc click). Test `emptyKey_showsFailedDialog` cũng fail tương tự (dialog không kịp hiện). Tái hiện 100% trên cả 2 thiết bị thật, cả code gốc lẫn code đã fix — chưa từng được verify trên thiết bị thật trước đây (có thể trước giờ chỉ chạy trên emulator). **Estimate: M** — cần investigate riêng, không nằm trong scope 7 P0 lần này.
- **FIX-039 — Quiz auto-advance race giữa `countDownTimer` (15s) và `postDelayed` (1.5s) dưới tải cao (P3).** Khi chạy đồng thời full suite 107 test trên cả 2 thiết bị cùng lúc, `testQuizSelectionAdvancesToNextQuestion` có 1 lần fail trên TECNO KJ7 (nhảy thẳng tới "4/10" thay vì "2/10", mất 36s thay vì ~2s) — dấu hiệu `countDownTimer` của câu hỏi cũ chưa kịp bị cancel trước khi hết 15s trên máy quá tải, dẫn tới `handleTimesUp()` tự động advance thêm ngoài ý muốn. Chạy riêng lẻ (không đồng thời) thì pass 100% (verify 3/3 lần trên baseline + 1/1 trên cả 2 thiết bị với code đã fix) — nên đây là race hiếm dưới tải cao, không phải lỗi logic đơn thuần. **Estimate: S** — cân nhắc thêm `countDownTimer?.cancel()` ngay đầu `checkAnswer()`/`handleTimesUp()` để loại bỏ hẳn race này.
