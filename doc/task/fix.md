...
- **FIX-040 — `privacyPolicyFooter_isDisplayedAndClickable` + `revoke_cancelled_vipCardStillVisible` fail trên Samsung SM-S928B (One UI) — ⚪ Cần verify, chưa fix.** Lần đầu test trên thiết bị này (sau Sprint 2). Lỗi: `getGlobalVisibleRect() returned empty rectangle` dù `visibility=VISIBLE`, x/y hợp lệ. Tái hiện 2/2 lần khi chạy cô lập riêng 2 test này (không phải flake ngẫu nhiên). Logcat cho thấy sự kiện ẩn IME (`onRequestHide`) xảy ra đúng lúc assertion chạy — nghi ngờ đây là race giữa animation ẩn bàn phím/window-insets và thời điểm Espresso đo `getGlobalVisibleRect()`, đặc thù edge-to-edge của One UI (chưa gặp ở TECNO KJ7/Pixel 7 Pro). **Không phải do Sprint 2:** `VipManagementAct.kt`/`VipManagementActTest.kt` không nằm trong diff Sprint 2 (lần sửa gần nhất là FIX-038, commit `34bd78b`). Việc TECNO KJ7 lúc chạy cùng đợt fail 3 test khác (`NoActivityResumedException`) cũng đã xác minh là do app khác (`com.saigonphantomlabs.chess`, một session Claude Code khác đang chạy song song) cướp focus trên cùng máy thật — không phải bug code. Cần điều tra thêm (thử `closeSoftKeyboard()` trước assertion, hoặc `IdlingResource` cho insets animation) trước khi sửa. — Nghi ngờ race giữa `countDownTimer` (15s) và `postDelayed` (1.5s) — ❌ Không phải bug, đã điều tra và bác bỏ.** Đọc lại `checkAnswer()` (`QuizAct.kt:421`) xác nhận `countDownTimer?.cancel()` đã được gọi **ngay dòng đầu tiên** khi trả lời — không có khoảng hở nào cho timer cũ tự bắn tiếp. Lần fail duy nhất quan sát được (`testQuizSelectionAdvancesToNextQuestion` nhảy tới "4/10" sau 36s) xảy ra khi chạy đồng thời toàn bộ 107 test trên cả 2 thiết bị cùng lúc — bản chất là: khi máy quá tải, toàn bộ quá trình thực thi test (bao gồm cả `Thread.sleep(2000)` và bước kiểm tra) bị trì hoãn tới mức thời gian thực trôi qua tới 36 giây; trong khoảng đó, cơ chế đếm giờ 15s/câu (vốn hoạt động đúng thiết kế) tự động advance thêm 2 câu do không có tương tác nào khác. Verify: chạy cô lập test này 3/3 lần trên code gốc + 1/1 lần trên cả 2 thiết bị với code đã sửa → pass tuyệt đối. Không sửa code app vì không có bug thật để sửa — sửa nhầm chỗ này sẽ là thay đổi không cần thiết.
